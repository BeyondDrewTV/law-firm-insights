...
## Active Pass
- **Pass:** Clarion AI Memory System Setup
- **Focus:** Establish a lightweight, repo-grounded AI handoff layer (startup files + guardrails + current build scope).

## Build Focus (Now)
1. Keep core product execution stable (upload → analysis → governance outputs).
2. Keep calibration workflow reliable and auditable.
3. Improve marketing/landing clarity without reimagining core dashboard workflows.

## Last Completed Pass (from git history)
- `b9a8a97` — calibration wave 2 phrase additions + bug fixes.

## Next Pass (Suggested)
- Landing/marketing clarity pass: sharpen conversion messaging while preserving existing product positioning and workflow claims.
