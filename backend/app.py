...
def create_onetime_checkout():
    ...
    try:
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            # PR3b: bind session to user so finalize/success can verify ownership

            client_reference_id=str(current_user.id),

            metadata={'user_id': str(current_user.id), 'plan': 'onetime'},
        ...
        _log_security_event(current_user.id, 'billing_checkout_created', metadata={'plan': 'onetime', 'channel': 'web'},
    ...
    try:
        ...
        if checkout_session.payment_status == 'paid' and str(checkout_session.customer) == str(current_user.stripe_customer_id):
            ...
            if ref_id and ref_id != str(current_user.id):
                ...
                _log_security_event(current_user.id, 'billing_checkout_mismatch', metadata={'route': 'onetime_success', 'session_id': session_id, 'reason': 'client_reference_mismatch'},
            ...
            if not ref_id and meta_uid and meta_uid != str(current_user.id):
                ...
                _log_security_event(current_user.id, 'billing_checkout_mismatch', metadata={'route': 'onetime_success', 'session_id': session_id, 'reason': 'metadata_user_mismatch'},
            ...
            try:
                ...
                # F4: dedupe gate � rowcount==0 means already processed
                ...
                if c.rowcount == 0:
                    ...
                    _log_security_event(current_user.id, 'billing_checkout_finalized', metadata={'plan': 'onetime', 'session_id': session_id, 'channel': 'web'},

                    )

                    # PR3b: do NOT claim a credit was just added

                    flash('Your payment was already processed and your report credit is ready to use.', 'success')
...
def create_subscription_checkout():
    ...
    try:
        ...
        _log_security_event(current_user.id, 'billing_checkout_created', metadata={'plan': plan, 'channel': 'web'},
...
def subscription_success():
    ...
    try:
        ...
        _set_firm_plan_for_user(c, current_user.id, plan)
